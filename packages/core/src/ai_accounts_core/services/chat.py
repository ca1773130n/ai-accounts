...
class ChatService:
    ...
    async def create_session(self, *, backend_id: str, model: str, title: str | None = None) -> ChatSession:
        await self._account_service.get(backend_id)
        session = ChatSession(id=new_id("cht"), backend_id=backend_id, title=title, created_at=_now(), model=model)
    ...
    async def send_message(
        self, *, session_id: str, content: str, role: ChatRole = ChatRole.USER,
        ...
        if accumulated_text:
            assistant_msg = ChatMessage(
                id=new_id("msg"), session_id=session_id, role=ChatRole.ASSISTANT,
                content=accumulated_text, created_at=_now(),
                model=model_used or session.model, tokens_in=tokens_in, tokens_out=tokens_out,
            )
