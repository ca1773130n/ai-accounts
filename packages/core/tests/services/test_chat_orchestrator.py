...
@pytest.mark.asyncio
async def test_send_single_emits_tool_call_event(tmp_path):
    """When a backend's chat() yields a tool-call ChatStreamEvent, orchestrator yields ToolCallEvent."""
    storage = FakeStorage()
    vault = FakeVault()
    fake = FakeBackend(
        tool_call={
    ...
    events = []
    async for event in orch.send_single(session_id=session.id, content="trigger tool"):
        events.append(event)
